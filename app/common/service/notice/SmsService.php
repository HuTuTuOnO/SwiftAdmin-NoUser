...
class SmsService
{
    ...
    public static function send($mobile, $event): bool
    {
        ...
        if (in_array($event, ['register', 'change']) && $userinfo) {
            ...
            throw new OperateException('当前手机号未注册');
        }

        if (!Event::hasListener('smsMsgSend')) {
            throw new OperateException('短信插件未安装');
        }
        ...
        if (!isset($smsConf[$smsType][$event]['template'])) {
            throw new OperateException('短信模板错误');
        }

        $response = Event::trigger('smsMsgSend', [
            'mobile'   => $mobile,
            'event'    => $event,
        ...
        if (isset($response['error']) && $response['error']) {
            throw new OperateException($response['error']);
        }
