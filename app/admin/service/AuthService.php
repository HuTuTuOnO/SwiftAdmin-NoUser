...
class AuthService
{
     ...
     * @throws ModelNotFoundException
     */
    public function getAuthList(int $adminId = 0, array $nodes = []): array
    {
        // 查找节点
        $where[] = ['status', '=', 'normal'];
        if (!$this->superAdmin()) {
    ...
    public function getPermissionsMenu(): string
    {
        ...
        $nodeLists = $this->getAuthList($this->adminInfo['id'], $authNodes);
    ...
    public function getRuleCatesTree($type, $class, bool $tree = true)
    {
        ...
        $authNodes = $this->getRulesNode($this->adminInfo['id'], $type);
        $where[] = ['status', '=', 'normal'];
...
//                $menuList = Model::field($this->authFields)->whereIn('id', $authNodes[$class])->where('status', 1)->select()->toArray();
            } else {
//                $menuList = Model::field($this->authFields)->where('status', 1)->select()->toArray();
            }
     ...
     * @throws ModelNotFoundException
     */
    public function getRulesNode(int $adminId = 0, string $type = AdminEnum::ADMIN_AUTH_RULES): array
    {
        $authGroup = $authPrivate = [];
        $adminId = $adminId > 0 ? $adminId : $this->adminInfo['id'];
    ...
    {
        $adminId = $adminId > 1 ? $adminId : $this->adminInfo['id'];
