...
class gitee
{
     ...
     * 用户登录
     */
    public function login(): \think\response\Redirect
    {
    ...
    public function getUserInfo(array $params = []): array
    {
        ...
        if ($access_token) {
            ...
            // 获取用户信息
            $ret = $this->http->get(self::GET_USERINFO_URL . '?' . http_build_query(["access_token" => $access_token]))->getBody()->getContents();
    ...
    public function getAccessToken(string $code = ''): array
    {
        ...
        try {
            ...
            if (str_contains($th->getMessage(), 'error_description')) {
